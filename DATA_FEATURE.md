...
- [ ] 5. Flag Party rounds, **two** of them: `superlative-gdp` and `superlative-gdppc`. `partyRounds/superlative.js` is **already generalized** (`createSuperlativeRound(createMetric(m, []), id)`; the skill's "known gap" note is stale). **Density is the exact worked example** (grep `densityRound` / `superlative-density`). Per round, mirror it across six spots: (a) `flags/partyRounds/superlative.js`, import the metric json + `export const gdpRound = createSuperlativeRound(createMetric(gdp, []), 'superlative-gdp')`; (b) `flags/partyRounds/superlative.test.js`, a round-instance test; (c) `party/partyGameServer.js` line ~40 `ROUNDS` array, add `superlative.gdpRound`; (d) `flags/partyPlan.js` `METRIC_MODES` entry + `flags/partyPlan.test.js` assertions; (e) `flagParty/page.js`, add `MODE_LABELS`, `SUPERLATIVE_MODES`, and an icon SVG; (f) i18n en+pl `party.mode.superlativeGdp` / `party.modeShort.superlativeGdp` (plus the per-capita pair). **Watch:** the per-capita "richest" round surfaces Vatican #1 ($375K estimate artifact) then Monaco / Liechtenstein, accurate to the data, just worth expecting.
