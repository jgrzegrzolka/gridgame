...
4. **Content** *(next — blob, no PR).* Author the family into `.catalog/puzzles.json` via the daily flow (pull → compute rosters with `resolveSuperlative` → hand-write titles/descriptions → `npm test` + `audit-superlative` → show diff → push): world top-10, per-continent top-5/7, Oceania top-5 most + top-5 smallest, and the compound ideas. Space per Jan's scheduling notes. Ships only after Phases 1-3 deploy.
