...
- [x] 3 + 4 (landed together, per Feature DJ's `THRESHOLD_METRICS` refactor note). `coffee(op, n)` factory + `COFFEE_BREAKS_FOR_RANDOM` (**atLeast-only** `>=1K/10K/100K tonnes`, 52/33/14 growers) + `tonnesCompact`/`tonnesToken` helpers + `THRESHOLD_METRICS.coffee` registry entry (`has: c => typeof c.coffee === 'number'`, `family: 'coffee'`). **Coffee is NOT 9×9-eligible** — too sparse/concentrated (whole continents grow none), so no break carries `ultimate: true`; every one is `ultimateEligible: false` and coffee stays a 3×3-only axis (pinned by an engine test). `attachCoffees` at the 3×3 load sites (party/server.js + online/offline 3×3 pages + findFlag + flagsdata); the three 9×9 sites are deliberately skipped since coffee can't appear there. Filters: `CoffeeConstraint` + `coffee` field on `Filters` (`matchesFilters` is generic); findFlag chooser section + `selectCoffee` + `coffeePills` + `coffeeProbability` random modifier + `chooserI18n` relabel + `findflag-random-coverage` skill note; flagsdata `buildMetricGroup('coffee', …)`. i18n `coffee.atLeast.{1k,10k,100k}` (en+pl) + `findFlag.sections.coffee` (reused by both surfaces). Tests: factory/round-trip/labelFor/atLeast-only/own-family/pool-membership/no-ultimate + random reachability + updated pool-count + seed-sweep attach. **Verified in-browser** (CDP cache-clear): flagsdata coffee filter group → "over 100K tonnes" = exactly 14 growers (Brazil…Vietnam; CAR/Guinea correctly excluded by the overrides); findFlag "Coffee production" chooser section; offline TTT 400/400 clean generation, ~26% carried a coffee axis; no-data guard blocks the ASEAN org while Germany (coffee 0) stays a fair wrong guess. (PR #___)
