...
| `logic-yetanotherquiz-release-daily` | Logic App (Consumption) | West Europe | **Disabled 2026-06-16 as part of Feature P Phase 2 cutover** — the timer-triggered `func-yetanotherquiz-release` now owns the midnight job. Kept around for ~1 week as a rollback path: re-enable via `az resource update --resource-type Microsoft.Logic/workflows --name logic-yetanotherquiz-release-daily --set properties.state=Enabled` and the workflow + Logic App revert to handling tonight's release. Resource removed by a follow-up PR once Function-based releases have a track record. Template: `logicapp-release-daily.bicep`. |
| `func-yetanotherquiz-release` | Function App (Consumption Y1, Linux Node 22) | West Europe | Timer-triggered daily-puzzle promoter. Fires at 00:05 Warsaw (`WEBSITE_TIME_ZONE=Central European Standard Time`), reads `catalog/live.json` + `catalog/backlog.json` from `styetanotherquiz` via managed identity, moves `backlog[0]` to the end of `live`, runs `validateCatalog()` (drift detector + sequential `n` + sovereign codes + en/pl descriptions), writes both blobs back. Failures surface in App Insights `ai-yetanotherquiz-release`. Source: `infra/release-fn/src/` (bundled via `node scripts/build-release-fn.mjs`). Template: `infra/funcapp-release-daily.bicep`. Manual invoke for smoke test: `curl -X POST -H "x-functions-key: <master>" -H "Content-Type: application/json" -d '{}' https://func-yetanotherquiz-release.azurewebsites.net/admin/functions/releaseDaily`. |
...
| `plan-yetanotherquiz-release` | App Service plan (Y1 Dynamic) | West Europe | Consumption-tier plan hosting `func-yetanotherquiz-release`. Free grant covers our ~31 invocations/month. |
| `ai-yetanotherquiz-release` | Application Insights | West Europe | Failure + invocation telemetry for `func-yetanotherquiz-release`. |
| `styetanotherquiz` | Storage account (Standard LRS, StorageV2) | West Europe | Hosts the public-read **`catalog`** container — `live.json` + `backlog.json` + `ideas.json` + `parked.json` + `policy.json`. Anonymous blob-level read, blob versioning ON, `Cache-Control: max-age=60`. CORS allows GET from `https://www.yetanotherquiz.com`, `https://yetanotherquiz.com`, `http://localhost:4280`, `http://localhost:7071`. Mutated by `func-yetanotherquiz-release`'s managed identity (Phase 2). The `gh-actions-yetanotherquiz-release` SP also has `Storage Blob Data Contributor` here as the Phase 1 fallback path. |
| `gh-actions-yetanotherquiz-release` | AAD app + service principal | (tenant) | Federated identity used by the (now dormant) `release-daily.yml` workflow to authenticate to Azure (OIDC). Federated credential subject `repo:jgrzegrzolka/gridgame:ref:refs/heads/main`, audience `api://AzureADTokenExchange`. Role: `Storage Blob Data Contributor` on `styetanotherquiz` only. Kept active during the Phase 2 rollback window. |
...
| `AZURE_CLIENT_ID` / `AZURE_TENANT_ID` / `AZURE_SUBSCRIPTION_ID` | GitHub repo secrets | `.github/workflows/release-daily.yml` (`azure/login@v2` OIDC + `az storage blob upload`) | Never — federated credential, no client secret to expire | Rotate only if the AAD app needs replacing. Re-create the federated credential (`subject: repo:jgrzegrzolka/gridgame:ref:refs/heads/main`) on the new app and update the three repo secrets. |
