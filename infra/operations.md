...
| `cosmos-yetanotherquiz-jg` | Cosmos DB NoSQL (Free tier ON) | West Europe | Containers under db `yetanotherquiz`, **all autoscale 100–1000 RU/s**: **`dailyResults`** — one row per (puzzle, deviceId) submission, `defaultTtl: 31_536_000` (1 year, set 2026-06-11 per FEATURE.md Feature F). **`quizRecords`** — one row per deviceId carrying every quiz config's PB + engagement counters (Feature F5), no TTL. **`profiles`** — one row per deviceId carrying the optional nickname (Feature H2), no TTL. **`tttPairs`** — one row per (deviceId, opponentId) per perspective carrying running head-to-head wins/losses/draws for both TTT modes (Feature G), no TTL. **`dailyLeaderboards`** — one row per (deviceId, configKey, UTC-date) carrying that day's leaderboard entry for a flag-quiz config (Feature K), partition key `/pk` = `"<configKey>\|<YYYY-MM-DD>"`, `defaultTtl: 691_200` (192 h — one day of buffer over the 168 h rolling window `quizLeaderboard.js` reads; **corrected 2026-07-17**, this said `172_800` / 48 h, which never matched the live container: verified via `az cosmosdb sql container show`, and the oldest live row was 188.7 h old, which only 192 h explains. A 48 h TTL would have silently reduced the 7-day board to 2 days). Composite indexes on `(score, durationMs)` both directions, specified at create time in `infra/dailyLeaderboards-index-policy.json`. Autoscale floor of 10% × 1000 = 100 RU/s per container × 5 containers = 500 RU/s baseline, well inside the 1000 RU/s Free Tier discount; you only pay when the sum of hourly peaks across containers exceeds 1000. The €5/month budget catches drift if that ever happens. **(The `passkeys` container that briefly existed during the original Feature C passkey design was deleted 2026-06-15 when Feature C pivoted to a QR-claim flow — see FEATURE.md Done. The `engagementEvents` container (Feature M Part B) was deleted 2026-06-23 when Feature S Phase 6 finished moving engagement counters to localStorage + syncBlob — see FEATURE.md.)** |
| `styetanotherquiz` | Storage account (Standard LRS, StorageV2) | West Europe | Hosts the public-read **`catalog`** container — `puzzles.json` + `ideas.json` + `parked.json` + `policy.json` (Feature R replaced the old `live.json` + `backlog.json` split with the single dated `puzzles.json`). Anonymous blob-level read, blob versioning ON, `Cache-Control: max-age=60`. CORS allows GET from `https://www.yetanotherquiz.com`, `https://yetanotherquiz.com`, `http://localhost:4280`, `http://localhost:7071`. Mutated by `authoring/push.mjs` from Jan's machine (fetches storage account key via `az`). |
...
Three secrets keep the runtime working — none expire on a clock since Feature P Phase 2 retired the GitHub-PAT-driven release path.
