...
| `styetanotherquiz` | Storage account (Standard LRS, StorageV2) | West Europe | Hosts the public-read **`catalog`** container — `live.json` + `backlog.json` + `ideas.json` + `parked.json` + `policy.json`. Anonymous blob-level read, blob versioning ON, `Cache-Control: max-age=60`. CORS allows GET from `https://www.yetanotherquiz.com`, `https://yetanotherquiz.com`, `http://localhost:4280`, `http://localhost:7071`. Updated by the `release-daily.yml` workflow's "Publish catalog to blob" step (Phase 1 of Feature P) so player-facing puzzle release is decoupled from the SWA deploy. |
| `gh-actions-yetanotherquiz-release` | AAD app + service principal | (tenant) | Federated identity used by `release-daily.yml` to authenticate to Azure (OIDC). Federated credential subject `repo:jgrzegrzolka/gridgame:ref:refs/heads/main`, audience `api://AzureADTokenExchange`. Role: `Storage Blob Data Contributor` on `styetanotherquiz` only. No client secret — nothing to rotate. |
