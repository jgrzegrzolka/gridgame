...
**Fix.** Added `scripts/build-webp.mjs` (sharp at **400 px wide, q90**) that converts every `flags/svg/*.svg` to `flags/webp/{code}.webp`. Run via `npm run build:webp`, output committed to the repo (same pattern as the SVGs themselves). Every thumb-sized consumer swapped from SVG → WebP: daily in-game + result + extra-stats, findFlag tiles, flagsdata grid, flagQuiz choices, ticTacToe cells (3×3 + 9×9, online + offline), home-page tile previews. **Zoom dialogs kept SVG** so click-to-expand still gets vector quality at full screen. Total catalog: SVG 2221 KiB → WebP 1480 KiB (67 %); worst offenders still shrank ~90-97 % (rs 178 → 12 KB; sh-ta 289 → 14 KB; bo 101 → 4 KB).

**Width / quality tuning.** First pass at 300 px / q80 nailed the bandwidth target (catalog → 810 KiB) but visibly pixelated flags with fine geometric detail at thumbnail size — GB diagonal cross, KR trigram bars, CN star edges, JP red-circle edge. Bumping to 400 px / q90 lands the cleaner downscale and keeps the WebP encoder from smoothing high-contrast edges. Catalog roughly doubles but is still 33 % smaller than the SVG source and well under the daily-result-stall threshold that triggered the work.
