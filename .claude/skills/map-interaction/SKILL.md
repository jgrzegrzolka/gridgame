...
- **Pan**: drag follows the finger. At an edge it **rubber-bands** (stretches past with growing resistance, capped at `MAX_OVERSCROLL = 0.2` of the view) and **springs back** on release with a slight overshoot bounce (`easeOutBack`, `SPRING_MS = 300`, `SPRING_OVERSHOOT`). Applies at *every* pan edge: the zoomed-in map edge (#741) and the zoomed-out floating-map keep-sliver edge (#742). (Tuned rubberier + springier 2026-07-09, Feature T: 0.12 -> 0.2 overscroll, easeOutCubic -> easeOutBack spring-back.)
