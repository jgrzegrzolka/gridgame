...
- **Pan**: drag follows the finger. At an edge it **rubber-bands** (stretches past with growing resistance, capped at `MAX_OVERSCROLL = 0.3` of the view) and **springs back** on release via a **real damped-spring integration** (`springStep`, per axis), seeded with the release flick velocity so the throw carries a touch further and settles organically. Params `SPRING_TENSION = 8` (ω) and `SPRING_DAMPING = 0.8` (ζ). Applies at *every* pan edge: the zoomed-in map edge (#741) and the zoomed-out floating-map keep-sliver edge (#742). History (all 2026-07-09, Feature T): overscroll 0.12 → 0.2 → 0.35 → 0.3; spring-back easeOutCubic-tween → easeOutBack-tween (SPRING_MS 260→300→560) → **physics spring** (`springStep`, tension 8 / damping 0.8), because a fixed-duration tween ignores release speed and read as "not smooth." The physics params were dialled in a standalone "rubber-band feel lab" artifact (drag a tile against walls, A/B spring vs tween, watch the return-curve trace) before porting — that's the pattern for tuning feel without churning the real map.
...
- **"Drag room" = rubber-band stretch, NOT a stable margin.** How the "let me drag it a bit past the edge" ask resolved after two wrong turns (both reverted 2026-07-09): (1) `edgePad` padded the *mounted* viewBox — rejected, it moved the default framing so the map rested with ocean around it. (2) `boundsExpand` (+ a `mounted`/`original` split, `expandBounds`) grew the *clamp region* so you could drag past the map and **rest** in the ocean — also rejected: Jan dragged the map down and it sat there with blank space below Antarctica (screenshot). What he actually wanted: the map always clamps to its tight framing, and "more drag room" means a **stretchier rubber-band that springs back**. So the final answer is just the tight clamp (`original` = mounted view, no expansion) plus a bigger `MAX_OVERSCROLL` and slower `SPRING_MS`. Lesson: "allow more dragging" on a bounded map means more overscroll give, not a larger reachable area.
