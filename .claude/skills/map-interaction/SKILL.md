...
- **Pan**: drag follows the finger. At an edge it **rubber-bands** (stretches past with growing resistance, capped at `MAX_OVERSCROLL = 0.12` of the view) and **springs back** on release (`easeOutCubic`, `SPRING_MS`). Applies at *every* pan edge: the zoomed-in map edge (#741) and the zoomed-out floating-map keep-sliver edge (#742).
...
- **Zoom** (wheel / pinch): pivot stays under the cursor/fingers. Hard-clamped at `MAX_ZOOM_IN = 24` and `MAX_ZOOM_OUT = 3` (no rubber — see rejected item).
