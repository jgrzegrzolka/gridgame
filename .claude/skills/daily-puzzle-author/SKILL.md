...
**Result-screen rank badges + captions (every metric, automatic).** The result grids badge each answer tile with its place in the ranking (`#1 #2 #3`, top-left) and a compact figure (top-right), and the flag-zoom caption carries the figure plus the country's world rank among sovereign states (e.g. "Population: 124.5 million · #12 in the world", "Area: 16,376,870 km² · #1 in the world"). This is computed live, not baked: `daily/page.js` resolves the entry's metric file via `metricFileFor` (`flags/superlativeRank.js`), fetches it, and installs captions for the **whole sovereign pool** via `setZoomNotes`. Covering the whole pool — not just the frozen answers — is the point: the result's **"Most missed" rail** (`extraStats` `topMistake`, the community's most-common *wrong* clicks) then captions the big-but-not-top-N distractors too, so a player who guessed a #12 country sees why it wasn't in the top 10. You do **not** author any of this.

**It works for whatever metric you pick — do not add a per-metric branch.** This used to be population-only, and puzzle #51 ("the 10 largest countries by area", 2026-07-26) shipped with no numbering at all because the gate read `metric === 'population'`. The numbering is the mechanic of a superlative, not a population perk. Two tests hold the line: `every superlative in the catalog can render its rank badges` (walks every entry, released and future-dated) and `the daily result screen fetches a superlative metric by registry, not by name` (fails if a page hardcodes one metric's data file). The one thing that stays metric-specific is caption *wording*: population phrases figures as "124.5 million", every other metric prefixes the entry's baked `notes` (or, for an unbaked distractor, a formatted figure + the metric's unit). So keep baking per-answer `notes` — they're the caption's first line, and the rank is appended to them.
...
**The dominant-continent check (run it on every new metric).** Before authoring a metric's 14, resolve each continent set and compare it to the world set of the same direction. When one continent owns a global ranking, its top-N comes back as *exactly* the world top-N — a pure recall puzzle, and a strict subset. This is not a rare edge: it has fired on four of the five metrics authored so far (`gdpPerCapita Europe most`, `gdp` and `elevation` for Asia, `temperature Africa most`). Two fixes, both proven:
- **Invert it** — replace the continent set with a `filter: "continent:!<name>"` variant ("the 10 highest peaks outside Asia"). Usually the better puzzle, since it surfaces the runners-up the world set hides.
- **Grow it** — author the continent at a larger topN so it reaches past the world set's tail, then pair it with the inverted set. Worth it when both halves tell a story (temperature: the Sahel at 10, plus the Gulf + atolls outside Africa).

**The tied-boundary check (run it on every `least` set).** `resolveSuperlative` breaks ties by country code, so a metric with a pile of identical values at one end produces a roster picked alphabetically rather than by the metric. Coastline is the worst case: 41 landlocked sovereigns sit at exactly 0 km, so "the 7 European countries with the shortest coastline" returns seven countries with *no* coastline, in code order. Resolve the set and look at the values before authoring; if the boundary is a tie, either scope to a region without the tied pile (Oceania and North America have no landlocked members) or turn the tied pile into its own manual puzzle ("The landlocked countries of Europe"). The same applies to any `absence: 'zero'` metric where the zeros are real.

Also compare every candidate against the **whole catalog**, not just the same metric: small continents collide across metrics by construction. Oceania has 14 sovereigns, so its most/least sets have already produced identical answer sets across population, area, density, GDP, elevation and temperature. An identical set is a dud puzzle even past #100, where rule 6 stops applying.
...
5. **Primary-clean colours (puzzles #1–100).** *(primary-clean test)* The same answer set must resolve under `{ colorField: 'primaryColors' }` as under the default. No emblem-only colour matches in onboarding. *Why:* a player typing Bolivia in "South America · blue" and getting accepted because Bolivia's blue is only in its coat of arms reads as "the game is wrong". First 100 puzzles are where players build trust; surprises here are uniquely expensive. Past #100: still preferred, not enforced.
   ...
   **Escape hatch: `primaryCleanExempt: true`.** Add this field on a puzzle entry to opt that single entry out of the test. Use it sparingly — every exempt puzzle is a place a player may type a flag and feel "the game is wrong". Pattern when adding: include a note in the entry's `description` field (or below in this rule) explaining *which* flag drifts and *why* keeping it is worth the trust cost. Current exemptions:
   ...
   - `#13 continent:Africa,color:yellow,color:red,color:green,motif:star-or-moon` and `#28 continent:Africa,motif:star-or-moon,color:!yellow` — **both hinge on Equatorial Guinea's emblem-only yellow** (`gq.additionalColors: ["yellow"]`, the gold stars + arms detail). #13 *includes* gq for a yellow the player can't really see; #28 *excludes* it for the same invisible yellow. Exempted rather than rewritten because both are already-released past puzzles (2026-06-18 / 2026-07-03) and rule 1's drift detector forces the answer sets to match the live resolution. Known trust cost, accepted deliberately: a player may type Equatorial Guinea on either and disagree with the verdict. If a *future* puzzle keys on `color:yellow` in Africa, prefer a framing that doesn't put gq on the boundary.
