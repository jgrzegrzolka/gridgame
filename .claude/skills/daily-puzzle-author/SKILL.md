...
**Result-screen captions (population only, for now).** On a `metric: "population"` superlative, the flag-zoom on the result screen shows each flag's population **and** its world rank among sovereign states (e.g. "Population: 124.5 million · #12 in the world"). This is computed live, not baked: `daily/page.js` fetches `population.json` for a population superlative and installs captions for the **whole sovereign pool** via `setZoomNotes` (`flags/populationRank.js` builds them). Covering the whole pool — not just the frozen answers — is the point: the result's **"Most missed" rail** (`extraStats` `topMistake`, the community's most-common *wrong* clicks) then captions the big-but-not-top-N distractors too, so a player who guessed a #12 country sees why it wasn't in the top 10. You do **not** author these — they're automatic for population superlatives. **This is metric-specific code.** A superlative on a *different* metric still works (rendering, scoring, audit) but shows only the plain flag name on zoom until someone adds a `build<Metric>RankNotes` + a branch in `daily/page.js` (or generalizes `populationRank.js` to take a metric key + label). The optional per-answer `notes` you can still bake into any entry act as an offline fallback.
