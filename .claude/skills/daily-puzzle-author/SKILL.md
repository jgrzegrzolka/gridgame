...
6. **No strict-subset puzzles (puzzles #1–100).** *(no-subset test, ideas-no-subset test)* For any two puzzles in #1–100, neither's answer set may be a strict subset, strict superset, or exact equal of the other's. **One of the most load-bearing rules** — violations completely undermine the "find all the X" framing because the player has already seen every answer in the smaller set when they met the larger one. *Why:* `Europe + cross` (live #1, 9 flags) followed by `Europe + cross + white` (8 of those 9) reads as "remember which of those 9 were white" — that's a memory puzzle, not a discovery puzzle. The player thinks the game is repeating itself. Past #100 allowed as a deliberate recall mechanic.
   ...
   *Enforcement is three-layered, all of it test-pinned or generator-enforced:*
   ...
   - **Ideas test** — fires on every non-parked entry in `daily_ideas.json`, against (live ∪ backlog ∪ other non-parked ideas). Parked ideas (`parkUntilN: 101`) are exempt — they're documented violators meant for past-#100 use.
   - **Generator** (`scripts/generate-candidates.mjs`) enforces rule 6 within each batch — every new candidate is checked against (live ∪ backlog ∪ everything already accepted in the current run), so subset chains can't form inside a generation pass.
