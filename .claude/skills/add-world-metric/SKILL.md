...
- **Result-screen rank badges + captions: zero new code.** `flags/superlativeRank.js` resolves any metric via `metricFileFor` and formats from the metric file's own `format` / `unit`, so the `#1 #2 #3` tile badges, the value pills and the "· #N in the world" zoom captions work the moment a metric is in `METRIC_FILES`. **Do not add a per-metric branch** — this was population-only until puzzle #51 ("largest by area") shipped with no numbering at all, and two tests in `flags/daily.test.js` now fail if a page hardcodes one metric's data file. The only per-metric authoring left is baking per-answer `notes` on the entry (the caption's first line, which the rank is appended to).
