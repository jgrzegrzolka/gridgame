...
## Iteration 16 — the host picks the opening round — BUILT (2026-07-20)

Round 1 was always Flags, decided by nobody. The host chooses it in the lobby now (defaulting to
Flags, so a host who ignores the control gets exactly the old game), **and the choice counts as their
first pick** — that second half is what makes it fair rather than a host perk.

**Picture modes only.** The opener is the warm-up everyone plays before any score exists, so it has to
be a round nobody has to think about; opening a party on "Honey production" is a bad first impression.
It also keeps the control a four-segment row instead of a scrolling list of thirty, so the lobby stays
one glance. `validateOpenerMode` enforces it, and coerces anything unrecognised — including the
nothing an older client sends — back to Flags.

**The mechanism is one seeded array.** `applyStart` puts the host into `pickedBy`, and `pickerFor`
already serves whoever has the fewest picks, so the rotation skips them for a cycle with no special
case anywhere downstream. A solo host still picks every round, because the minimum simply moves up
with them.

### The cost, which is real and was accepted deliberately

`LENGTH_ROUNDS` cells were hand-chosen so `rounds - 2` divided evenly by the seat count — that is what
let the lobby hint say "you each pick 2" and be *literally* true. With the opener counted the total is
`rounds - 1`, which does not divide, so `pickShareFor`'s `extra` branch — built for 7+ seats and rare
until now — becomes the normal case. The copy for it already existed (`party.lengthEachPicksPlus`), so
nothing new had to be written.

The alternative was re-picking all 13 cells by ±1 to land on the new lattice. Rejected for now: it
would move every game's length and invalidate the ~10/20/30-minute modelling that Iteration 14
deliberately left unmeasured. Cheap to revisit; changing lengths is not.

Two pinned tests changed from claims that are no longer true, rather than being loosened:
`the Decider: the rotation is spent evenly` cannot tie at two seats any more (odd total), so it asserts
**within one** and states why; and the duo rotation test now expects the *losing* seat to pick twice
running, because after the host's opener the two are level and a tie goes to the lower-ranked seat.
That is the catch-up the seeding exists for.

### Surface

A second segmented row under the length, reusing `.dl-step` / `.dl-pick` rather than growing a parallel
set of classes, with each segment wearing its own mode's artwork — the same icon the draft hand and the
round card use, so a host learns one picture per round type. It carries a visible heading
(`.dl-group-label`, "Opening round") because once two segmented rows stack, neither one's labels
identify it on their own. Guests see it read-only in the same place, like the length: the opening round
is something they are told, not something withheld.

Room state `opener` parallels `length` exactly — same null-means-nobody-set-it contract, same
