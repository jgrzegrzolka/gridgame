...
- [x] **Phase 4b-ii — the Facts deck.** *(2026-07-17, Jan: "yes".)* **Feature V is complete.** The superlative deck ships in flagQuiz: "Most cattle per person?" over four flags, 60s-only, world/sovereign pool, from the same catalog and round factory Flag Party uses.
  ...
  - **The rule-application moved into `superlativeCore.buildSuperlativeRound(entry, raw)`** — the zero-filter + direction lock + `createMetric`, previously inline in `superlative.js`. Two consumers now share it (the server, the Facts deck) and neither re-decides what `zeroFiltered` means, which is the drift 4b-i existed to stop. `superlative.js` still produces byte-identical output (re-verified: 1920 questions vs main).
