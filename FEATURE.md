...
- [x] **Phase 4b-i — extract the metric catalog. Behaviour-neutral, shipped alone.** *(2026-07-17.)* `flags/partyRounds/superlativeCatalog.js` — a data-free, browser-safe table of the 32 metrics carrying the three facts a Facts round needs and **no metric data can supply**: the `direction` lock (coffee asks "biggest producer" only), the `zeroFiltered` flag, and the hint copy. Phase 4a split the *logic* from the data; this splits the *rules* from it, which turned out to be the other half of the same problem — 4a alone left Facts unable to ask a correct question.
  ...
  - **Verified by driving the real page**, not by reading: a live room dealt a real superlative round that rendered "Largest coal production" with coal's icon and its `#424242` hue, `data-metric="superlative-coal"`, four flags — the whole changed path (catalog lookup → `hintFor` → `metricKeyForRound` → hue/icon). The catalog also loads in a real browser and resolves all 32. *(That last part is worth what 4a's probe was worth and no more: Chromium loads the JSON-importing half happily too. The static import-graph test is the only real protection.)*
- [x] **Phase 4b-i-fix — make direction part of the staleness check.** *(2026-07-17, Jan: "yes, do it".)* 4b-i's review found `hintFor`'s fallback was reachable and shipped **silent mis-scoring**; it was documented rather than fixed, and Jan asked for the real fix.
  ...
  - Also pinned: a question with **no** `roundId` (it's optional on the wire type) stays unrenderable, which is what `Set.has(undefined)` already did — the old call site only typechecked because `tsconfig.ui.json` has `strictNullChecks` off.
- [ ] **Phase 4b-ii — the Facts deck.** `VARIANTS.facts` + a `decks.js` entry, fetching the metric JSON and handing it to `createSuperlativeRound` with the catalog's rules. Two things earlier phases deferred to here: **(1)** `availableModes()` finally needs its variant argument — Facts is the only deck with nothing to exhaust, so it's 60s-only, and this is where that rule lands. **(2)** The prompt is **not a country name**, it's a superlative ("Most forest cover?"), so `.country-name` needs a second prompt kind. Art stays `'flag'` — the choices are four flags, as in Flag Party. Pool is **sovereign** (see 4b-i's uninhabited-territories finding). `decks.test.js` currently asserts `facts` is the one icon with no deck; that test failing is it working — update it.
