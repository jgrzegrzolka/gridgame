...
- [ ] **Phase 4b — the Facts deck.** `VARIANTS.facts` + a `decks.js` entry, fetching the metric JSON and handing it to `createSuperlativeRound`. Two things earlier phases deferred to here: **(1)** `availableModes()` finally needs its variant argument — Facts is the only deck with nothing to exhaust, so it's 60s-only, and this is where that rule lands. **(2)** The prompt is **not a country name**, it's a superlative ("Most forest cover?"), so `.country-name` needs a second prompt kind. Art stays `'flag'` — the choices are four flags, as in Flag Party.
