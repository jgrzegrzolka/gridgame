...
**Follow-up: `superlativeCore.js` folded back into `superlative.js`.** *(PR #1131, separate on purpose — it touches Flag Party's server-side question generation, where a regression should be obvious rather than buried in a flagQuiz deletion.)* Phase 4a split the quartet-picking logic into its own file for exactly one reason: to be JSON-free so **a browser** could load it, and the browser was the Facts deck. With the deck gone the core had one consumer left — `superlative.js`, which runs server-side and can hold the code inline. Its `superlativeCore.test.js` JSON-import guard was protecting a constraint nothing depended on any more, so it went too; `superlativeCatalog.js` is the browser-safe half now and the repo-wide `page.js` walk already covers it. `createSuperlativeQuestion` stays exported so the synthetic-metric tests can build fake metrics — it is not in `party/partyGameServer.js`'s registry, which maps over an explicit list of the per-metric exports rather than the namespace. **Proved behaviour-neutral by diffing 4680 dealt questions** (39 metrics × 60 seeds × two exclude shapes) against `main`: byte-identical.
...
> **Superseded in part by Feature X (2026-08-02):** Outlines and Statistics were removed — nobody played either — along with the per-variant `art` / `ask` / `endless` / `modes` machinery they were the only consumers of. **Phase 4a's `superlativeCore.js` split went with them**, since its whole purpose was to be browser-loadable for the Facts deck; it is folded back into `superlative.js`. `weird`, the scope-toggle deletion, and the Phase 4b-i catalog all stand. Everything below describes the state as shipped in July; read Feature X for what is actually on the page now.
...
**Step 2 — tile states (4f). Shipped, PR #1138.** One border channel, on the flag itself. `padding: 7px` with no border reproduces the old `5px + 2px transparent border` exactly, so the flag does not move — only the ring it wears. `pulse-*` → `ring-*`, renamed because the keyframes now carry the solid ring as well as the halo. Hover and press deleted from **both** flag games. Flag Party's `.opt.sel` (no flagQuiz equivalent, not covered by the canvas) moved onto the media too, rather than leave that game with two competing ring systems. Everything pinned to the flag's edge shifted 2px inward (name band, `.opt-pop`, `.picks`).

**Step 3 — result screens (4d, 4g). Shipped, PR #1139.** The headline is whatever actually varied: the score ordinarily, the clock once the pool fell inside the budget. `resultView.js` owns the choice. Two bugs it exposed and fixed: a give-up banked a sub-budget time (so a forfeit beat a full round of the same score in `nextBest`, and the screen offered "record 5/195 in 0:08" as though the pool had been swept — `timedBudgetUsedMs` now spends the whole budget on a give-up); and `.result-cleared ~ .result-headline` matched while `hidden`, shrinking every ordinary score to the clean-sweep size.

**Step 4 — stats rebuild (5b, 5c). Shipped, PR #1140.** `flagQuiz/stats/` re-read as pool coverage: one measure per row (the 60s record) as number, bar and sort key, so the bottom of the list is what to practise. Four row states in `statsView.js`. No per-row "Play" and no mode legend — the row is the tap target. The world sits above the list, since the other seven pools are slices of it. The page's implementation also left its inline `<script type="module">` for a sibling `stats/page.js`.

**The canvas is complete.** Nothing in `Yetanotherquiz 60s Flow Current.dc.html` is outstanding: 4e and 5a were reproductions of the then-current production state rather than asks, and every other panel has shipped.

#### Progress log

- **2026-08-03 (3)** — Steps 2, 3 and 4 built and shipped (#1138, #1139, #1140), completing the canvas. Each verified in a real browser: the tile ring measured at a 7px flag inset before and after (the flag must not move), both result screens driven for real (South America cleared inside the budget for the sweep; 5-right/1-wrong then give up for the ordinary one), and all four stats row states seeded onto one screen. Two habits paid off and are worth keeping: **measuring instead of looking** caught both step-3 bugs, one of which (`~` matching a `hidden` sibling) would have read as plausible in a screenshot; and **registering every new pure module in `tsconfig.json`** caught a null-deref in `sortCoverageRows` and a possibly-`undefined` return in `resolveRoundConfig`. Known unrelated issue found along the way: **the lobby's "add bot" seat does not seat a bot on the local stack** (the PartyKit socket connects — `101 Switching Protocols` — but the lobby stays at one player and Play stays disabled), which blocked a live Flag Party round for the step-2 verify; the tiles were checked by measuring the real markup against the live stylesheet instead.
