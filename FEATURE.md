...
- [x] **N1 — pure compute + tests.** `api/src/lib/streakCompute.js`: takes a sorted `[{puzzleId, completed}]` array (+ optional `latestPuzzleId`), returns `{currentStreak, maxStreak, winPercent, totalPlayed, totalCompleted}`. 13 unit tests covering empty / single / all-completed / all-missed / mid-broken / gap-without-failed-row / streak-still-alive / missed-today / unsorted / rounding / no-mutation. *(Originally landed at `flags/streakCompute.js` in PR #438; relocated to `api/src/lib/` during N2 — every caller is server-side, the frontend just reads the rendered numbers, and `flags/` would not be packaged into the deployed Functions bundle anyway. CJS to match api/ convention.)*
- [x] **N2 — read endpoint.** `GET /api/v1/daily/me?deviceId=…&latestPuzzleId=…` in `api/src/functions/dailyMe.js`. Cross-partition `SELECT c.puzzleId FROM c WHERE c.deviceId = @did` against `dailyResults`, maps to `[{puzzleId, completed:true}]`, calls `computeStreak`. 60s in-Function cache keyed by `deviceId|latestPuzzleId`, `?fresh=1` bypass, 60/min/IP rate limit, `local:true` rows included (own player's local plays are their own plays). `latestPuzzleId` is optional — the client knows today's puzzleId and passes it so `currentStreak` resets to 0 when a player skipped today; if absent, falls back to "trailing run ending at most recent row" (fine for finish-screen use right after submit). No new container, no Cosmos schema change.
- [x] **N3 — finish screen wiring.** New `daily/streakClient.js` (`fetchDailyMe`, never-throws, defensive shape normalisation, 13 tests). `daily/page.js` fires the fetch after both the natural-finish pipeline and the revisit paint; result lands in a module-level `streakState` that `paintStatsPanel` reads on every repaint (rebuild-on-each-paint pattern, same as the share button). Renders `Day streak: {n}` as one quiet line under the headline only when `currentStreak ≥ 2`. New `.daily-stats-streak` CSS rule — same primary-colour family as headline, opacity 0.75, weight 500, 14px — clearly secondary, no badge / card / box. i18n strings under `daily.streak.line` in `en.json` + `pl.json`. Soft language switch re-renders correctly because the label resolves at paint time.
