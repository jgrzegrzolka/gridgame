...
**`DAILY_RESULT_UPSERT` toggle (now off in prod, 2026-06-10).** When `true`, `dailyResult` upserts into Cosmos instead of insert-only — replays update the stored row. Was ON during early Feature B testing so the stats panel reflected each player's latest replay, keeping localStorage and Cosmos in sync. **Flipped OFF** once `daily/scores.js` was switched to first-attempt-only (same PR as this note), so client and server now share the same "first attempt wins" rule end-to-end. To re-enable for testing, set it back to `true` via the Azure portal — no redeploy needed.
...
- **Local archive is first-attempt-only (as of 2026-06-10).** `daily/scores.js` `saveScore()` is a no-op when a record already exists for that N. Replays still play through (replay-skip-revisit is preserved) but don't overwrite the archive — mirrors the server's insert-only rule. Earlier behavior was "last-result wins"; flipped because a client/server semantic mismatch (Cosmos keeps first, local kept last) confused players in testing.
- **The client POSTs on every finish — including replays. No client-side gate.** An earlier version of `statsSubmit.js` gated on `hasSubmitted(n)` as a "don't waste a round-trip when we know it'll 409" optimization, but that gate prevented the server's upsert path from doing its job (the second POST never reached the server, so the row never got replaced). The gate is gone — the server is the source of truth for dedup in both modes (insert: 409 on duplicate; upsert: replaces). `submitted.js` is still used by `statsSubmit.js` to mark a puzzle as POSTed on 204/409 (bookkeeping for the finish-screen flow).
...
- Stats panel is gated on Cosmos, not this device's localStorage. The revisit branch in `page.js` always GETs `/api/v1/daily/stats/{n}` and lets the response decide: `totalAttempts === 0` → `formatScoreLine` falls back to score-only and `paintStatsPanel` skips the caption. Originally the revisit branch checked `hasSubmitted` so only players who'd POSTed would see stats — that excluded anyone who'd finished a puzzle on a different device or before submit-tracking shipped, and the play-to-see-stats incentive wasn't worth that gap. The finish branch in `handleFinish` still uses `submitResult`'s outcome to decide whether to call `loadAndPaintStats` with `bypassCache: true`.
