...
- [x] Function `releaseDaily.js` with timer binding. Reads live + backlog from blob via `@azure/storage-blob` + `DefaultAzureCredential`, calls `promote()` + `validateCatalog()`, writes back. Idempotent at the blob level (overwrite same key). Empty backlog throws — same "refill" cue as before. **Caveat:** schedule is hardcoded `0 5 22 * * *` (UTC) rather than `0 5 0 * * *` + `WEBSITE_TIME_ZONE`, because Linux Consumption ignores both `WEBSITE_TIME_ZONE` and the `CRON_TZ=Europe/Warsaw` prefix syntax (the indexer rejects the prefix). `22:05 UTC = 00:05 Warsaw during CEST`. **Manual DST bump:** flip to `0 5 23 * * *` at the end of October when CET starts; back to `0 5 22 * * *` at the end of March. Tracked as Phase 2.5 (proper fix: Premium plan that honours `WEBSITE_TIME_ZONE`, or two schedules with a runtime gate).
- [x] One-off invocation verifies end-to-end — promoted #12 to blob in 405 ms, blob then restored to pre-test state so tonight's auto-fire promotes #12 cleanly.
- [x] Disable Logic App (`az resource update --set properties.state=Disabled`) — kept the resource for ~1-week rollback window. Workflow file `.github/workflows/release-daily.yml` and `scripts/release-next.mjs` **stay in main** until the first natural midnight fire is verified; cleanup PR removes them tomorrow (avoids a single-PR cutover that has both the new path and the cleanup; if Phase 2 misbehaves tonight, re-enabling the Logic App is one `az` command + the workflow code is still there).

**Phase 2.5 — DST-resilient schedule.** *(Follow-up after Phase 2 is proven.)*
- [ ] Replace the UTC-hardcoded schedule with a TZ-aware fix. Options: (a) move to a Premium plan that honours `WEBSITE_TIME_ZONE`; (b) register two timers (one for CEST offset, one for CET) and runtime-skip the wrong one; (c) keep one timer and let the handler compute "is now within 1h of Warsaw midnight" before doing work. Pick after seeing how (a)'s cost lands — Premium is ~$15-20/mo on Consumption-comparable scale, vs $0 today.
