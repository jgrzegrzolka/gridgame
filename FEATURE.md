...
- **The client POSTs on every finish — including replays. No client-side gate.** An earlier version of `statsSubmit.js` gated on `hasSubmitted(n)` as a "don't waste a round-trip when we know it'll 409" optimization, but the gate suppressed legitimate re-sends (Turnstile glitch retries, etc.) — the second POST never reached the server. Server is the only correct source of truth for dedup. `submitted.js` is still used by `statsSubmit.js` to mark a puzzle as POSTed on 204/409 (bookkeeping for the finish-screen flow).
