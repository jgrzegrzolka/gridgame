...
- **Colours: only the four defined variables.** The palette is `var(--primary-color)` (near-black `#2B1D24`), `var(--secondary-color)` (pink `#C2899F`), `var(--muted-color)` (grey `#999`), and `var(--muted-soft-color)` (lighter grey `#ddd`, for subtle divider / hairline rules), all defined in `common.css`. Every `color:` / `background:` / `border-color:` in any CSS file must use one of these four (or a `var(--link-color)` alias which resolves to primary). No ad-hoc hex / rgb / named colours, no `opacity: 0.x` on primary to fake a fifth shade — if a tone reads "lighter than primary," that's muted; if it reads "even quieter, just a hint of a line," that's muted-soft; if it reads "accent," that's secondary. Exceptions: the per-tile bottom strip uses `rgba(0,0,0,0.7)` over a flag image (the flag colours come from the SVG), and feature-specific neutrals like `#888` or `#fff` inside SVG / canvas contexts. When adding any new colour, grep first for one of the four vars to reuse.
