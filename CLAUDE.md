...
The site's HTTP API lives in `api/` and ships as part of the SWA deploy (no separate Function App resource — it runs inside the SWA's managed Function App). A **second, standalone** Function App (`func-yetanotherquiz-release`, source in `infra/release-fn/`) handles the daily-puzzle promotion at Polish midnight; that one is a separate Azure resource because SWA-managed Functions don't support timer triggers. See `daily/README.md` for the daily flow and `infra/operations.md` for the resource inventory.
...
- **Colours: only the three defined variables.** The palette is `var(--primary-color)` (near-black `#2B1D24`), `var(--secondary-color)` (pink `#C2899F`), and `var(--muted-color)` (grey `#999`), all defined in `common.css`. Every `color:` / `background:` / `border-color:` in any CSS file must use one of these three (or a `var(--link-color)` alias which resolves to primary). No ad-hoc hex / rgb / named colours, no `opacity: 0.x` on primary to fake a fourth shade — if a tone reads "lighter than primary," that's muted; if it reads "accent," that's secondary. Exceptions: the per-tile bottom strip uses `rgba(0,0,0,0.7)` over a flag image (the flag colours come from the SVG), and feature-specific neutrals like `#888` or `#fff` inside SVG / canvas contexts. When adding any new colour, grep first for one of the three vars to reuse.
