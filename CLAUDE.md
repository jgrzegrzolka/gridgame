...
**Dev reset toolbar (localhost only):** `daily/devReset.js` mounts a small yellow toolbar bottom-left on the daily + archive pages when `window.location.hostname` is a localhost host. Two buttons: **Reset browser** wipes the daily-flow localStorage keys (`gridgame.deviceId`, `gridgame.submittedPuzzles`, `daily.scores`, `gridgame.ideas.reviewed`) and reloads; **Clear Cosmos local rows** POSTs to `/api/v1/dev/clear-local-rows` which deletes every `dailyResults` doc with `local: true`. The endpoint refuses anywhere but a localhost-bound runtime (same `isLocalRequestUrl` check that sets the tag). Use both together to start fresh during dev — replay any puzzle without "already submitted" 409s, and clean up the test rows you accumulated.
