...
async function renderAchievements(deviceId) {
  ...
  // Detect newly-earned achievements — the ones the player has just
  // unlocked since their last visit. Each newly-earned rule triggers
  // a drop-and-bounce celebration overlay; seen state persists in
  // localStorage so the celebration fires once per achievement, not
  // on every page revisit.
  //
  // On the *first* visit after this feature ships, an existing player
  // could have 7 already-earned badges — firing 7 celebrations in a
  // row reads as a glitch, not a reward. Silent-backfill: if there's
  // no `init` flag yet, mark everything currently earned as seen
  // without firing any animation. The next genuine unlock triggers
  // a celebration. Same idiom Feature O backlog discussion called
  // out (FEATURE.md: "silent backfill matches the calm Feature N tone").
  const seen = loadSeenAchievements();
  const isFirstVisit = !readSeenInitFlag();
  /** @type {import('../flags/achievements.js').AchievementRule[]} */
  const newlyEarned = isFirstVisit
    ? []
    : statuses
        .filter((s) => s.earned && !seen.has(s.rule.id))
        .map((s) => s.rule);
  ...
  // Mark all currently-earned ids as seen BEFORE firing celebrations
  // — if the player force-refreshes mid-animation, we still don't
  // repeat the unlock. The animation queue plays from the in-memory
  // copy. Also flip the init flag so the next visit's diff is
  // real-vs-seen instead of triggering the silent backfill again.
  for (const { rule, earned } of statuses) {
    if (earned) seen.add(rule.id);
  }
  saveSeenAchievements(seen);
  writeSeenInitFlag();

  if (newlyEarned.length > 0) void runCelebrationQueue(newlyEarned);
}

const SEEN_KEY = 'gridgame.achievements.seen';
const SEEN_INIT_KEY = 'gridgame.achievements.seenInit';

function readSeenInitFlag() {
  try { return window.localStorage.getItem(SEEN_INIT_KEY) === '1'; } catch { return false; }
}
function writeSeenInitFlag() {
  try { window.localStorage.setItem(SEEN_INIT_KEY, '1'); } catch { /* non-fatal */ }
}
...
function saveSeenAchievements(seen) {
  try {
    window.localStorage.setItem(SEEN_KEY, JSON.stringify(Array.from(seen)));
  } catch {
    // Quota / private-browsing — non-fatal. Celebration may re-fire
    // on next visit, which is a milder UX bug than throwing here.
  }
}

/**
 * Play the drop-and-bounce overlay for each newly-earned rule in
 * sequence — one at a time so they don't pile up on screen. Each
 * overlay holds for ~2.4s after the bounce settles, then fades out;
 * a click anywhere on the backdrop or the card dismisses early.
 *
 * @param {import('../flags/achievements.js').AchievementRule[]} rules
 */
async function runCelebrationQueue(rules) {
...
function celebrateOne(rule) {
  return new Promise((resolve) => {
    const overlay = document.getElementById('ach-celebrate');
    const card = document.getElementById('ach-celebrate-card');
    const iconEl = document.getElementById('ach-celebrate-icon');
    const nameEl = document.getElementById('ach-celebrate-name');
    const descEl = document.getElementById('ach-celebrate-desc');
    if (!overlay || !card || !iconEl || !nameEl || !descEl) {
      resolve();
      return;
    }
    ...
    // between queued celebrations (the second rule with the same
    // animation declaration on the element wouldn't otherwise replay).
    overlay.classList.remove('ach-celebrate--leaving');
    ...
    const dismiss = () => {
      if (dismissed) return;
      dismissed = true;
      overlay.classList.add('ach-celebrate--leaving');
      // Wait for the fade-out keyframe (matches CSS .ach-celebrate--leaving).
      setTimeout(() => {
        overlay.hidden = true;
        overlay.classList.remove('ach-celebrate--leaving');
    ...
    /** @param {MouseEvent} ev */
    const onClick = (ev) => {
      // Backdrop OR card click both dismiss — this is a transient
      // notification, not a modal, so any tap means "I saw it."
      void ev;
      dismiss();
    };
